...
def run_LAmbDA(DataPath, LabelsPath, CV_RDataPath, OutputDir, GeneOrderPath = "", NumGenes = 0):
    ...
    truelab.to_csv("LAmbDA_true.csv", index = False)
    predlab.to_csv("LAmbDA_pred.csv", index = False)
    tr_time.to_csv("LAmbDA_training_time.csv", index = False)
    ts_time.to_csv("LAmbDA_test_time.csv", index = False)
...
if sys.argv[6] == "0":
    run_LAmbDA(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
else:
    run_LAmbDA(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])
