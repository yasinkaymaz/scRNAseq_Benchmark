...
def run_ACTINN(DataPath, LabelsPath, CV_RDataPath, OutputDir, GeneOrderPath = "", NumGenes = 0):
    ...
    for i in range(np.squeeze(nfolds)):
        ...
        os.system("python3 /Scripts/actinn_format.py -i train.csv -o train -f csv")
        os.system("python3 /Scripts/actinn_format.py -i test.csv -o test -f csv")

        start = tm.time()
        os.system("python3 /Scripts/actinn_predict.py -trs train.h5 -trl train_lab.csv -ts test.h5")
    ...
    truelab.to_csv("ACTINN_true.csv", index = False)
    pred.to_csv("ACTINN_pred.csv", index = False)
    tot_time.to_csv("ACTINN_total_time.csv", index = False)

if sys.argv[6] == "0":
    run_ACTINN(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
else:
    run_ACTINN(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])
