...
run_CaSTLe<-function(DataPath,LabelsPath,CV_RDataPath, OutputDir, GeneOrderPath = NULL, NumGenes = NULL){
  ...
  setwd(OutputDir)
    write.csv(True_Labels_Castle,'CaSTLe_true.csv',row.names = FALSE)
    write.csv(Pred_Labels_Castle,'CaSTLe_pred.csv',row.names = FALSE)
    write.csv(Training_Time_Castle,'CaSTLe_training_time.csv',row.names = FALSE)
    write.csv(Testing_Time_Castle,'CaSTLe_test_time.csv',row.names = FALSE)
}
